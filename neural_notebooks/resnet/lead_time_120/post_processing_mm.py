...
pred_ensemble_1 = np.load('/rds/general/user/mc4117/ephemeral/saved_pred_120/whole_res_more_data_do_19_2.npy')
pred_ensemble_2 = np.load('/rds/general/user/mc4117/ephemeral/saved_pred_120/whole_res_more_data_do_19_3.npy')
...
preds_1 = xr.Dataset({
    'z': xr.DataArray(
        ...
        coords={'time':dg_test.data.time[120:], 'lat': dg_test.data.lat, 'lon': dg_test.data.lon, 'ens': np.arange(samples), 
                },
    ),
    't': xr.DataArray(
        pred_ensemble_1[1, ...],
...
preds_2 = xr.Dataset({
    'z': xr.DataArray(
        pred_ensemble_2[0, ...],
        dims=['time', 'lat', 'lon', 'ens'],
        coords={'time':dg_test.data.time[120:], 'lat': dg_test.data.lat, 'lon': dg_test.data.lon, 'ens': 12+ np.arange(samples), 
                },
    ),
    't': xr.DataArray(
        pred_ensemble_2[1, ...],
...
sample_list = [i for i in range(1,24)]
import random
random_list = random.sample(sample_list, 23)
