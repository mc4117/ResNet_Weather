...
if var_name == 'specific_humidity':
    cnn_2 = build_cnn([64, 64, 64, 64, 2], [5, 5, 5, 5, 5], (32, 64, 4))
elif var_name == 'pot_vort':
    cnn_2 = build_cnn([64, 64, 64, 64, 2], [5, 5, 5, 5, 5], (32, 64, 4))
elif var_name == 'const':
    cnn_2 = build_cnn([64, 64, 64, 64, 2], [5, 5, 5, 5, 5], (32, 64, 5))
elif var_name == 'orig':
    cnn = build_cnn([64, 64, 64, 64, 2], [5, 5, 5, 5, 5], (32, 64, 2))
else:
    cnn_2 = build_cnn([64, 64, 64, 64, 2], [5, 5, 5, 5, 5], (32, 64, 3))
    
if var_name != 'orig':
    cnn_2.load_weights('/rds/general/user/mc4117/home/WeatherBench/saved_models/72_gpu_annualorig.h5')
    j = 2
    i = 2
    for k in range(2, len(cnn.layers)):
        if type(cnn.layers[j]) == type(cnn_2.layers[i]):
            cnn.layers[j].set_weights(cnn_2.layers[i].get_weights())
            j += 1
            i += 1
