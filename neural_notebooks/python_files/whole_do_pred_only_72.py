...
for m in range(number_of_forecasts):
    print(m)
    output = create_predictions(cnn_2, dg_test)
    pred2 = np.asarray(output.to_array(), dtype=np.float32).squeeze()
    pred_ensemble[:,:,:,:,m]=pred2
    np.save('/rds/general/user/mc4117/home/WeatherBench/saved_pred/whole_bn_no_do_72_2.npy', pred_ensemble)
