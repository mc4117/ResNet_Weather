...
class DataGenerator(keras.utils.Sequence):
    def __init__(self, ds, var_dict, lead_time, batch_size=32, shuffle=True, load=True, mean=None, std=None):
        ...
        self.binned_data = xr.Dataset({'z': xr.DataArray(
               to_categorical(np.digitize(ds.z, bins_z)-1, dtype = 'int8', num_classes = 100),
...
def build_resnet_cnn(filters, kernels, input_shape, l2 = None, dr = 0, skip = True):
    ...
    output = Softmax(dtype = 'int8')(output_conv)
...
def my_loss(yTrue,yPred):
    ...
    return  K.mean(K.equal(yTrue_reshape, yPred_reshape))

cnn.compile(keras.optimizers.Adam(5e-5), loss=my_loss)
...
cnn.load_weights('/rds/general/user/mc4117/home/WeatherBench/saved_models/whole_cat_no_ohe_' + str(block_no) + '.h5')
...
preds_cat_inv = np.array(list(map(lambda x: dg_test.bins_z[int(preds_cat_int[x])], itertools.product(range(nx), range(ny), range(nz)))))
...
print(compute_weighted_rmse(full_out, real_ds).compute())
