...
class DataGenerator(keras.utils.Sequence):
    def __init__(self, ds, var_dict, lead_time, batch_size=32, shuffle=True, load=True, mean=None, std=None):
        ...
        self.binned_data = xr.Dataset({'z': xr.DataArray(
               to_categorical(np.digitize(ds.z, self.bins_z)-1, num_classes = 100, dtype = 'int8'),
...
real_ds = xr.DataArray(y1_full, dims=['time', 'lat', 'lon'], coords={'time':dg_test.data.time[72:], 'lat': dg_test.data.lat, 'lon': dg_test.data.lon})
