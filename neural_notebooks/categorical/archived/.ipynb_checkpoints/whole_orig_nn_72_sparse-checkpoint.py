...
class DataGenerator(keras.utils.Sequence):
    def __init__(self, ds, var_dict, lead_time, batch_size=32, shuffle=True, load=True, mean=None, std=None):
        ...
        digitized_sparse = np.zeros((nx, ny, nz, 1))
        ...
        for i in range(nx):
            ...
            for j in range(ny):
                nz_list = []
                for k in range(nz):
                    nz_list.append(sp.sparse.csc_matrix(([1], ([0], [digitized_z[i][j][k]])), shape=(1, 100), dtype = 'int8'))
                ny_list.append(nz_list)
            nx_list.append(ny_list)

        sparse_data = np.array(nx_list)
