...
dg_valid = DataGenerator(
    ds_valid, var_dict, lead_time, batch_size=bs, shuffle=False, output_vars = output_vars)
...
def indices_to_one_hot(data, nb_classes):
    ...
    return np.eye(nb_classes)[np.array(data).reshape(-1)]    

one_hot_y1 = indices_to_one_hot(y1, 100).reshape(17472, 32, 64, 100)
...
def my_init(shape, dtype=None):
    print(shape)
    return tf.ones(shape, dtype=dtype)/400
...
def build_stack_model(input_shape, stack_list):
    ...
    x = Dense(400, kernel_initializer = my_init)(merge)
    x = ReLU(max_value=1)(x)
    out = Dense(100)(x)
    out = ReLU(max_value=1)(out)    
...
ensemble_model = build_stack_model((32, 64, 100), stack_test_list)
...
ensemble_model.fit(x = stack_test_list, y = one_hot_y1, epochs = 600, validation_split = 0.2, shuffle = True, verbose =2,
                  callbacks = [early_stopping_callback, reduce_lr_callback
                    ])

ensemble_model.save_weights('stacked_cat_9gpu.h5')