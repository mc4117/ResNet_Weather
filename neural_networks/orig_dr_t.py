# Trains residual neural network with dropout on categorical T850 output data for 3-day forecast. The number of blocks is specified in the command line
...
dg_valid = DataGenerator(
    ds_train.sel(time=slice('2015', '2015')), dic, lead_time, batch_size=bs, mean=dg_train.mean, std=dg_train.std, bins_t = dg_train.bins_t, shuffle=False)
...
cnn.save_weights('/rds/general/user/mc4117/home/WeatherBench/saved_models/whole_cat_crossent_non_ohe_dr_t_' + str(block_no) + '.h5')
...
for i in range(no_of_forecasts):
    ...
    fc_arg_avg = np.dot(fc, dg_test.bins_t)
...
for i in range(len(fc_all)):
    fc_avg += fc_all[i]
    cnn_rmse_arg = compute_weighted_rmse(fc_avg/(i+1), ds_test.t[72:]).compute()
