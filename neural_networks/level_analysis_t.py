# Consider original data of z500 and t850 with one other variable to predict T850 3-day forecast. Can specify the levels of this one other variable that are included
...
ds_test = ds_whole.sel(time=slice('2016', '2016'))
...
dg_valid = DataGenerator(
    ds_train.sel(time=slice('2015', '2015')), var_dict, lead_time, batch_size=bs, mean=dg_train.mean, std=dg_train.std, bins_t = dg_train.bins_t, shuffle=False, output_vars = output_vars)
...
filename = '/rds/general/user/mc4117/ephemeral/saved_models/whole_cat_do_t_' + str(args.block_no) + '_' + str(var_name) + str(unique_list)
...
for i in range(len(fc_all)):
    fc_avg += fc_all[i]
    cnn_rmse_arg = compute_weighted_rmse(fc_avg/(i+1), ds_test.t.sel(level = 850)[72:]).compute()
